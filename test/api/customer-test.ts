...
describe("Customer tests", () => {
  ...
  it("Get customer by Id", async () => {
    ...
    const response = await agent.get("http://ec2-13-58-210-183.us-east-2.compute.amazonaws.com:8080/api/customer/" + id)
      .set("Content-type", "application/json")
      .set("Accept", "application/json");
  ...
  it("Get customer by name", async () => {
    it("A costumer must be show", async () => {
      const response = await agent.get(
        "http://ec2-13-58-210-183.us-east-2.compute.amazonaws.com:8080/api/customer/username=" + customerName
      )
  ...
  it("Get Customer by Username", async () => {
    ...
    const response = await agent.get(
      "http://ec2-13-58-210-183.us-east-2.compute.amazonaws.com:8080/api/customer/username=" + username
    )
      .set("Content-type", "application/json")
      .set("Accept", "application/json");
    chai1.expect(response.status).to.equal(StatusCodes.OK);
  ...
  it("Update customer", async () => {
    it("The correct costumer should be update", async () => {
      ...
      const response = await agent.put(
        "http://ec2-13-58-210-183.us-east-2.compute.amazonaws.com:8080/api/customer/" + customerTestId
      )
        .set("Content-type", "application/json")
        .set("Accept", "application/json")
        .send(customer);
  ...
  it("Delete customer by Id", async () => {
    const id = customerTestId; 
    const response = await agent.del("http://ec2-13-58-210-183.us-east-2.compute.amazonaws.com:8080/api/customer/" + id) 
      .set("Content-type", "application/json")
      .set("Accept", "application/json");
  ...
  it("Delete all customers in system", async () => {
    const response = await agent.del(`http://ec2-13-58-210-183.us-east-2.compute.amazonaws.com:8080/api/customer/`)
      .set("User-Agent", "agent")
      .set("Content-Type", "application/json");
    chai1.expect(response.status).to.equal(StatusCodes.NO_CONTENT);
  });
