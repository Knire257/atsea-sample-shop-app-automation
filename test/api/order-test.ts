...
describe("Order Tests", () => {
  ...
  it("Create an order", async () => {
    const response = await
      agent.post("http://localhost:8080/api/order/")
        .send({
          orderId: 1,
  ...
  it("Get all orders", async () => {
    const response = await
      agent.get("http://localhost:8080/api/order/")
        .set("Content-type", "application/json")
        .set("Accept", "application/json");
  ...
  it("Get order by Id", async () => {
    ...
    const response = await
      agent.get("http://localhost:8080/api/order/" + id)
        .set("Content-type", "application/json")
        .set("Accept", "application/json");
    expect(response.status).to.equal(StatusCodes.OK);
    expect(response.body).to.have.property("orderId");
  ...
  it("Delete order by Id", async () => {
    const id = orderTestId;
    const response = await
      agent.del("http://localhost:8080/api/order/" + id)
        .set("Content-type", "application/json")
        .set("Accept", "application/json");
